...
export default function Toolbar() {
  ...
  const handleSave = useCallback(async () => {
    ...
    } catch (err) { toast.error(`Save failed: ${err?.response?.data?.error || err.message || 'Unknown error'}`); }
  }, [t]);
  ...
  const handleLoad = useCallback(async () => {
    ...
    } catch (err) { toast.error(`Load failed: ${err?.response?.data?.error || err.message || 'Unknown error'}`); }
  }, [t]);
