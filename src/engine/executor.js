...
/**
 * Execute a code node (inline JavaScript evaluated in a sandbox).
 * WARNING: This is NOT sandboxed for production. Use vm module with proper isolation.
 */
async function executeCodeNode(node, inputData, onLog, timeout) {
  const code = node.data?.code || '';
  if (!code) return inputData || {};

  // Simple eval-based execution 鈥?replace with vm2/isolated-vm for production
  const fn = new Function('input', 'onLog', code);
  const result = fn(inputData, onLog);
  return result ?? {};
}
