...
async function _executeAndRecord(node, ctx, results) {
  ...
  try {
    ...
    if (continueOnError) {
      onLog('warn', `⚠ ${node.data?.label || node.id} 失败但已跳过（标记为可选节点）: ${err.message}`);
    } else {
      onLog('error', `✗ ${node.data?.label || node.id} 失败: ${err.message}`);
    }
  ...
  while (queue.length > 0) {
    ...
    for (const node of currentBatch) {
      ...
      if (useStepMode) {
        ...
        if (action === 'stop') {
          onLog('warn', `执行已停止（用户中止于节点 ${node.data?.label || node.id}）`);
        ...
        if (action === 'skip') {
          onLog('info', `⊘ ${node.data?.label || node.id} 已跳过`);
...
async function withRetry(fn, maxRetries, delayMs, label, onLog) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      if (attempt > 0) onLog('warn', `🔄 重试 "${label}" (${attempt}/${maxRetries})`);
      return await fn();
    } catch (err) {
      lastError = err;
      if (attempt < maxRetries) {
        onLog('warn', `⚠ "${label}" 失败: ${err.message}，${delayMs}ms 后重试...`);
        await sleep(delayMs);
      }
    }
  }
  onLog('error', `✗ "${label}" 在 ${maxRetries + 1} 次尝试后仍然失败: ${lastError.message}`);
  throw lastError;
}
